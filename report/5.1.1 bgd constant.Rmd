...
```{python bgd_constant_learning_rate_gs_python, echo=F, eval=T}
bgd_constant_summary = pd.read_csv(os.path.join(directory, bgd_constant_summary_filename))
bgd_constant_detail = pd.read_csv(os.path.join(directory, bgd_constant_detail_filename))
bgd_constant_report = pd.read_csv(os.path.join(directory, bgd_constant_report_filename))
```
...
viz.figure(alg=alg, data=data, x='learning_rate', y='duration', 
           func=viz.lineplot, directory=directory, 
           filename=filename, width=0.5)         
```
...
bgd_constant_search_demo_filename = 'Batch Gradient Descent Constant Learning Rate Search Path.gif'
bgd_constant_fit_demo_filename = 'Batch Gradient Descent Constant Learning Rate Regression Fit.gif'
```

```{python bgd_constant_learning_rate_solution, echo=F, eval=T, cache=F}
theta = np.array([-1,-1]) 
learning_rate_sched = 'c'
learning_rate = bgd_constant_report['learning_rate'].iloc[0]
precision = bgd_constant_report['precision'].iloc[0]
maxiter = bgd_constant_report['maxiter'].iloc[0]
i_s=bgd_constant_report['i_s'].iloc[0]

bgd_constant_lr_demo = BGDDemo()
bgd_constant_lr_demo.fit(X=X,y=y, theta=theta, learning_rate=learning_rate, 
         learning_rate_sched=learning_rate_sched,
         precision=precision, maxiter=maxiter, 
         i_s=i_s)

bgd_constant_lr_demo.show_search(directory=directory, filename=bgd_constant_search_demo_filename, fps=1)
bgd_constant_lr_demo.show_fit(directory=directory, filename=bgd_constant_fit_demo_filename, fps=1)
```
...
```{python bgd_constant_learning_rate_gs_ii_python, echo=F, eval=T}
bgd_constant_summary_ii = pd.read_csv(os.path.join(directory, bgd_constant_summary_ii_filename))
bgd_constant_detail_ii = pd.read_csv(os.path.join(directory, bgd_constant_detail_ii_filename))
bgd_constant_report_ii = pd.read_csv(os.path.join(directory, bgd_constant_report_ii_filename))
```
...
theta = np.array([-1,-1])  
learning_rate_sched = bgd_constant_report_ii['learning_rate_sched'].iloc[0]
learning_rate = bgd_constant_report_ii['learning_rate'].iloc[0]
time_decay = bgd_constant_report_ii['time_decay'].iloc[0]
step_decay = bgd_constant_report_ii['step_decay'].iloc[0]
step_epochs = bgd_constant_report_ii['step_epochs'].iloc[0]
exp_decay = bgd_constant_report_ii['exp_decay'].iloc[0]
precision = bgd_constant_report_ii['precision'].iloc[0]
maxiter = bgd_constant_report_ii['maxiter'].iloc[0]
stop_metric = bgd_constant_report_ii['stop_metric'].iloc[0]
i_s=bgd_constant_report_ii['i_s'].iloc[0]

```

```{python bgd_constant_learning_rate_solution_ii_filenames, echo=F, eval=T}
bgd_constant_search_ii_demo_filename = 'Batch Gradient Descent Constant Learning Rate II Search Path.gif'
bgd_constant_fit_ii_demo_filename = 'Batch Gradient Descent Constant Learning Rate II Regression Fit.gif'
```

```{python bgd_constant_learning_rate_path_ii, echo=F, eval=T, cache=F}
bgd_constant_lr_demo = BGDDemo()  
bgd_constant_lr_demo.fit(X=X, y=y, theta=theta, X_val=X_val, y_val=y_val, n=500, 
            learning_rate_sched=learning_rate_sched, 
...
bgd_constant_lr_demo.show_search(directory=directory, filename=bgd_constant_search_ii_demo_filename, fps=1)
bgd_constant_lr_demo.show_fit(directory=directory, filename=bgd_constant_fit_ii_demo_filename, fps=1)
```
