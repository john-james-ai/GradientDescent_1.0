...
```{python bgd_time_gs, echo=F, eval=T, cache=T}
lab = BGDLab()
lab.gridsearch(X=X, y=y, X_val=X_val, y_val=y_val, theta=theta, learning_rate=learning_rate, 
               learning_rate_sched=learning_rate_sched, time_decay=time_decay, 
               step_decay=None, step_epochs=None, exp_decay=None,
...
```{python bgd_time_gs_results_python, echo=F, eval=T}
bgd_time_decay_summary = pd.read_csv(os.path.join(directory, bgd_time_decay_summary_filename))
bgd_time_decay_detail  = pd.read_csv(os.path.join(directory, bgd_time_decay_detail_filename))
bgd_time_decay_report  = pd.read_csv(os.path.join(directory, bgd_time_decay_report_filename))
...
bgd_time_decay_report_costs <- bgd_time_decay_report %>% arrange(final_costs, duration) %>% select(learning_rate, time_decay, final_costs, duration)
bgd_time_decay_report_error <- bgd_time_decay_report %>% arrange(final_mse, duration) %>% select(learning_rate, time_decay, final_mse)
```
...
```{python bgd_time_path_filenames, echo=F, eval=T}
bgd_time_decay_path_filename = 'Batch Gradient Descent - Time Decay Convergence.gif'
bgd_time_decay_fit_filename = 'Batch Gradient Descent - Time Decay Fit.gif'
```

```{python bgd_time_path, echo=F, eval=T, cache=T}
bgd_time_decay_demo = BGDDemo()
bgd_time_decay_demo.fit(X=X,y=y, theta=theta, learning_rate=learning_rate, 
         learning_rate_sched=learning_rate_sched, time_decay=time_decay,
         precision=precision, maxiter=maxiter, 
         i_s=i_s)
       
bgd_time_decay_demo.show_search(directory=directory, filename=bgd_time_decay_path_filename, fps=1)
bgd_time_decay_demo.show_fit(directory=directory, filename=bgd_time_decay_fit_filename, fps=1)
```
...
```{python bgd_bgd_time_decay_summary_results, echo=F, eval=T}
bgd = BGD()
...
`r kfigr::figr(label = "bgd_bgd_time_decay_summary_results", prefix = TRUE, link = TRUE, type="Figure")`: Batch Gradient Descent Time Decay Learning Rate Optimal Solution Results 
