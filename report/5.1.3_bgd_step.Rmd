...
The above two plots show training set costs (left) and validation set error (right) for our step decay learning rate parameters. Both plots have very similar shapes and appear to minimize at or near the same step decay value. As it turns out, the best performing value for the step decay hyparameter (`r bgd_step_decay_eval_train['step_decay'][1,]`) minimizes validation set error at `r round(bgd_step_decay_eval_train['final_mse'][1,],4)`. The best performing parameter sets from the training and error perspectives are listed below. 
...
Therefore, it would be sensible to infer that our solution generalizes well to unseen data. 
...
params <- bgd_step_decay_report %>% select(learning_rate, step_decay, step_epochs, precision, maxiter, i_s) 
