...
```{python bgd_exp_gs_results_python, echo=F, eval=T}
bgd_exp_decay_summary = pd.read_csv(os.path.join(directory, bgd_exp_decay_summary_filename))
bgd_exp_decay_detail  = pd.read_csv(os.path.join(directory, bgd_exp_decay_detail_filename))
bgd_exp_decay_report  = pd.read_csv(os.path.join(directory, bgd_exp_decay_report_filename))
...
```{python bgd_exp_final_params, echo=F, eval=T}
theta = np.array([-1,-1]) 
learning_rate_sched = 'e'
learning_rate = bgd_exp_decay_report['learning_rate'].iloc[0]
exp_decay = bgd_exp_decay_report['exp_decay'].iloc[0]
precision = bgd_exp_decay_report['precision'].iloc[0]
maxiter = bgd_exp_decay_report['maxiter'].iloc[0]
i_s=bgd_exp_decay_report['i_s'].iloc[0]
```

```{python bgd_exp_path_filenames, echo=F, eval=T, cache=F}
...
exp_decay_demo.show_search(directory=directory, filename=bgd_exp_decay_search_filename, fps=1)
exp_decay_demo.show_fit(directory=directory, filename=bgd_exp_decay_fit_filename, fps=1)
```
...
```{python bgd_exp_decay_summary_results, echo=F, eval=T}
bgd = BGD()
...
filename = "Batch Gradient Descent Exponential Decay Learning Rate Solution Results.png"            
bgd.plot(directory, filename, show=False)            
```

![](../report/figures/BGD/Lab/Batch Gradient Descent Exponential Decay Learning Rate Solution Results.png)
`r kfigr::figr(label = "bgd_exp_decay_summary_results", prefix = TRUE, link = TRUE, type="Figure")`: Batch Gradient Descent Exponential Decay Learning Rate Optimal Solution Results 
