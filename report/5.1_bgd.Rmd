...
`r kfigr::figr(label = "bgd_search_by_alpha_100", prefix = TRUE, link = TRUE, type="Figure")` shows the convergence path for learning rates 0.02, 0.1,and 0.8 for a minimum of 100 observations. 
...
The regression linesin `r kfigr::figr(label = "bgd_fit_by_alpha_100", prefix = TRUE, link = TRUE, type="Figure")` show a marked improvement over those in `r kfigr::figr(label = "bgd_fit_by_alpha", prefix = TRUE, link = TRUE, type="Figure")`. 
...
Here we see the effect of learning rate when the number of iterations is fixed. As we approach the ideal learning rate of 0.8, we see improvement in the regression fit. Learning rate 0.02 was too small to make adequate progress within 100 iterations.  The situation was slightly better for learning rate 0.1. We were able to obtain a reasonably good fit to the data with a learning rate of 0.8 over 100 iterations. 
...
* As a solution approaches convergence, the gradient becomes vanishingly small, effectively preventing the $\theta$'s from changing their values.  Consequently, changes in the gradient or objective function costs diminish to a value approaching zero and the algorithm stops.     
