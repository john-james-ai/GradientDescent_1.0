...
* The number of epochs has reached a preset maximum,     
* Convergence hasn't improved in $i_s$ consecutive epochs.
...
The SGD class will have a concrete implementation of the fit method to implement the algorithm described in `r kfigr::figr(label = "sgd_algorithm", prefix = TRUE, link = TRUE, type="Figure")`.
...
The shuffle method referenced below is defined as follows:
...
Next, we'll learn how to tune the hyperparameters for stochastic gradient descent.
...
* **stopping condition hyperparameters**:
    ...
    - $i_s$, the algorithm stops when the number of consecutive epochs of no improvement equals $i_s$.

Like batch gradient descent, stochastic gradient descent is most essential hyperparameter is learning rate. We will therefore use a gridsearch strategy to create candidate functions $f_θ(x)$ for a range of learning rate schedules and learning rates. The initial $\theta$’s, our precision parameter $\epsilon$, maxiter, and the $i_s$ parameter will be set to default values and adjusted as needed.
