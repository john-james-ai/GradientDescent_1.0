...
lab = SGDLab()
constant_summary = pd.read_csv(os.path.join(directory, constant_summary_filename))
constant_detail = pd.read_csv(os.path.join(directory, constant_detail_filename))
```

```{r sgd_constant_get_gs_r, echo=F, eval=T}
constant_summary = read.csv(file.path(py$directory, py$constant_summary_filename))
constant_detail = read.csv(file.path(py$directory, py$constant_detail_filename))
```
...
lab.figure(data=constant_summary, x='learning_rate', y='final_costs',
...
With that, let's drill down a bit and investigate the effect of our stopping criteria on the optimization problem vis-a-vis learning rate. 
...
y = ['final_costs']
assoc = lab.associations(data=constant_summary,x=x,y=y, directory=directory, filename=filename)
```
...
```
Based upon `r kfigr::figr(label = "sgd_constant_param_corr", prefix = TRUE, link = TRUE, type="Table")`, our stop criteria any association between our stop criteria and objective function performance is weak at best. 
...
Alas, we reveal the top 3 performing hyperparameters.
...
best_constant <- constant_summary %>% select(experiment, stop_metric_label, learning_rate, precision, i_s, maxiter,  epochs, iterations, duration, final_costs) %>% arrange(final_costs, duration)
knitr::kable(head(best_constant,3), digits = 4, format.args = list(big.mark = ",")) %>%
 kableExtra::kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = T, position = "center")
```

According to `r kfigr::figr(label = "sgd_constant_best", prefix = TRUE, link = TRUE, type="Table")`, we obtained a final objective function cost $J=$ `r round(best_constant['final_costs'][1,],4)` in `r best_constant['epochs'][1,]`, totalling `r round(best_constant['duration'][1,],4)` milliseconds, with a learning rate $\alpha=$ `r round(best_constant['learning_rate'][1,],2)`. The algorithm stopped when the change in `r tolower(best_constant['stop_metric_label'][1,])` dropped below `r best_constant['precision'][1,]` for `r best_constant['i_s'][1,]` consecutive epochs.

##### Solution Analysis
Let's examine our solution a bit more closely. 
