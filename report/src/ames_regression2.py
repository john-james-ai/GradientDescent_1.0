...
e_table_2 = pd.concat([df1, df2, df3], axis=1)
mse_row = pd.Series({"Sales Price ($)": "MSE", "Squared Error": mse},
                    index=e_table_2.columns).fillna("")
e_table_2 = e_table_2.append(mse_row, ignore_index=True)

# Extract slope and intercept
intercept = lr.intercept_[0]*.9
slope = lr.coef_[0][0]*1.12

# ============================================================================ #
#                                   PLOT                                       #
# ============================================================================ #
# Create error line coordinates
...
# Hypothesis Annotation Coordinates
h_pointx = 1000
h_pointy = intercept + slope * 1000
h_textx = 1000
h_texty = 140000
...
# Plot Hypothesis
ax.text(x=1000, y=220000, s=r'$h(x)=17,498.49 + 113.17x$')
...
plt.title("Housing Price by Area - Hypothesis 2")

plt.tight_layout()
# plt.show()
fig.savefig("./report/figures/ames_regression2.png", facecolor='w')
plt.close(fig)
